...
Console.WriteLine($"First customer name is {firstCustomerName} with Query<(int, string, string)>");

// the tuple can be deconstructed into its individual parts
foreach (var (id, name, color) in customers)
{
    Console.WriteLine($"Found customer {id} ({name}) [{color}] with Query<(int, string, string)>");
}
...
{
    Console.WriteLine($"Found customer {tuple.Id} ({tuple.Name}) [{tuple.FavoriteColor}] with Query<(int, string, string)>");
}
